...
void Scene::load(int id) {
	...
	if (id == 1) {
		...
		float pi = 3.1415926f;
		for (int i=0;i<n;i++) {
			bobbingSpheres.push_back(BobbingSphere(glm::vec3( 10.0f*(i%4) + 10.0f, 0.0f, 10.0f*(i/4) - 40.0f), 3.0f, glm::vec3(0.0f, 1.0f, 0.0f), -6.0f, 6.0f, 1.0f, i*2.0f*pi/(float)n, glm::vec4(colors1[i%4], 1.0f - i/4 * 0.333f), glm::vec4(0.1f, 0.5f, 0.5f, 32.0f)));
			bobbingSpheres.push_back(BobbingSphere(glm::vec3(-10.0f*(i%4) - 10.0f, 0.0f, 10.0f*(i/4) - 40.0f), 3.0f, glm::vec3(0.0f, 1.0f, 0.0f), -6.0f, 6.0f, 1.0f, i*2.0f*pi/(float)n, glm::vec4(colors2[i%4], 1.0f - i/4 * 0.333f), glm::vec4(0.1f, 0.5f, 0.5f, 32.0f)));
		}
		...
		int n = 30;
			float pi = 3.1415926f;
		for (int i=0;i<n;i++) {
			bobbingSpheres.push_back(BobbingSphere(glm::vec3(rnd(-50.0f, 50.0f), rnd(0.0f, 10.0f), rnd(-10.0f, -110.0f)), rnd(2.0f, 6.0f), glm::vec3(0.0f, 1.0f, 0.0f), rnd(-8.0f, -2.0f), rnd(2.0f, 8.0f), rnd(0.5f, 2.0f), rnd(0.0f, 2.0f*pi), glm::vec4(rnd(0.0f, 1.0f), rnd(0.0f, 1.0f), rnd(0.0f, 1.0f), 0.0f), glm::vec4(0.1f, 0.5f, 0.5f, 32.0f)));
		}
		...
		for (int i=0;i<30;i++) {
			...
			bobbingSpheres.push_back(BobbingSphere(glm::vec3(cos(phi)*n, 0.0f, -30.0f + sin(phi)*n), 1.0f, glm::vec3(0.0f, 1.0f, 0.0f), -10.0f, 10.0f, 1.0f, phi*3.0f, glm::vec4(rnd(0.0f, 1.0f), rnd(0.0f, 1.0f), rnd(0.0f, 1.0f), 0.0f), glm::vec4(0.1f, 0.5f, 0.5f, 32.0f)));
		}
		...
		for (int i=0;i<n;i++) {
			float pi = 3.1415926f;
			float phi = i * 2.0f*pi/(float)n;
			bobbingSpheres.push_back(BobbingSphere(glm::vec3(((n-1)/2.0f-i)*2.0f, 0.0f, -20.0f), 1.0f, glm::vec3(0.0f, cos(phi), sin(phi)), -10.0f, 10.0f, 1.0f, 24.0f*phi, glm::vec4(1.0f, 1.0f, 1.0f, 0.0f), glm::vec4(0.1f, 0.5f, 0.5f, 32.0f)));
		}
		...
		float pi = 3.1415926f;
		for (int i=0;i<n;i++) {
			bobbingSpheres.push_back(BobbingSphere(glm::vec3( 10.0f*(i%4) + 10.0f, 0.0f, 10.0f*(i/4) - 40.0f), 3.0f, glm::vec3(0.0f, 1.0f, 0.0f), -6.0f, 6.0f, 1.0f, i*2.0f*pi/(float)n, glm::vec4(colors1[i%4], 1.0f - i/4 * 0.333f), glm::vec4(0.5f, 0.5f, 0.5f, 32.0f)));
			bobbingSpheres.push_back(BobbingSphere(glm::vec3(-10.0f*(i%4) - 10.0f, 0.0f, 10.0f*(i/4) - 40.0f), 3.0f, glm::vec3(0.0f, 1.0f, 0.0f), -6.0f, 6.0f, 1.0f, i*2.0f*pi/(float)n, glm::vec4(colors2[i%4], 1.0f - i/4 * 0.333f), glm::vec4(0.5f, 0.5f, 0.5f, 32.0f)));
		}
